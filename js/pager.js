...
var kkpager = {
    ...
    generPageHtml: function(config, enforceInit) {
        ...
        if (this.isShowTotalPage || this.isShowTotalRecords) {
            ...
            if (this.isShowTotalPage) {
                total_info += config.total + this.lang.totalPageAfterText;
                if (this.isShowTotalRecords) {
                    total_info += '/';
                }
            }
            if (this.isShowTotalRecords) {
                total_info += config.totalRecords + this.lang.totalRecordsAfterText;
            }
    ...
    init: function(config) {
        ...
        if (this.pno < 1)
            this.pno = 1;
        this.total = (config.total <= 1) ? 1 : config.total;
        if (config.pno > config.total)
            this.pno = config.total;
        this.prv = (config.pno <= 2) ? 1 : (config.pno - 1);
        this.next = (config.pno >= config.total - 1) ? config.total : (config.pno + 1);
        this.hasPrv = (config.pno > 1);
        this.hasNext = (config.pno < config.total);

        this.inited = true;
    },
    _getHandlerStr: function(n) {
        if (this.mode == 'click') {
            return 'href="javascript:void(0)" onclick="return kkpager._clickHandler(' + n + ')"';
        }
        //link模式，也是默认的
